...
root = r"C:\Users\bruno\OneDrive\Desktop\BrainReader RESEARCH\Code\external_gits\StableDiffusion_img2img\stable-diffusion"
...
def run_demo(
    ...
    config = OmegaConf.load(os.path.join(root, config))
    model = load_model_from_config(config, os.path.join(root, ckpt), device=device)
